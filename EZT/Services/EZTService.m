...
+ (NSArray *)_parseNearByRestaurant:(id)jsonDic {
	if (!jsonDic) {
		...
		NSString *errorDomain = @"EZTServerErrorDomain";
		NSError *error = [NSError errorWithDomain:errorDomain code:[code integerValue] userInfo:@{NSLocalizedDescriptionKey: message}];
		[self handleError:error];
...
@implementation CLGeocoder (DelayedReverseGeocode)
static NSDate *lastQueryDate;
//TODO: DelayedReverseGeocode
+ (Promise *)delayedReverseGeocode:(CLLocation *)location {
	
	return [Promise new:^(PromiseResolver fulfiller, PromiseResolver rejecter) {
		
		VoidBlock queryBlock = ^{
			[CLGeocoder reverseGeocode:location].then(^(CLPlacemark *firstPlacemark){
				fulfiller(firstPlacemark);
			}).catch(^(NSError *error){
				rejecter(error);
			});
			lastQueryDate = [NSDate date];
			NSLog(@"LOG:  lastQueryDate.timeIntervalSince1970: %f",lastQueryDate.timeIntervalSince1970);
		};
		
		[self checkAndRun:queryBlock];
    }];
}

+ (void)checkAndRun:(VoidBlock)runBlock {
	NSDate *now = [NSDate date];
	
	BOOL immediately = (now.timeIntervalSince1970 - lastQueryDate.timeIntervalSince1970) >1;
	if (immediately || !lastQueryDate) {
		runBlock();
	}else {
		dispatchAfter(0.1, ^{
			[self checkAndRun:runBlock];
		});
	}
}
